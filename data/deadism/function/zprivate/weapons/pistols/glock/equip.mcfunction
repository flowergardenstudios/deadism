...
execute unless score @s deadism.weapon.AMMO.IN_GUN matches ..0 as @n[tag=deadism.weapon.NEW] run function animated_java:glock/animations/equip/play
#execute if score @s deadism.weapon.AMMO.IN_GUN matches ..0 if score @s deadism.weapon.AMMO.RELOADABLE_AMOUNT matches ..0 on passengers run function animated_java:glock/animations/equip_empty/play
